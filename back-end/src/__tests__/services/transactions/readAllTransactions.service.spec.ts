...
describe("List all transactions", () => {                      
    ...
    test("Should list all registered transactions", async () => { 
   ...
   `
    ...
    const transactionsString = transactions.split("\n")

    transactionsString.shift()
    transactionsString.pop()
    
    const transactionList: string[] = []

    transactionsString.map(item => transactionList.push(item.trim()))

    const transactionsStringHandled  = transactionList.toString().replace(/,/g, "\n")

    console.log(transactionsStringHandled)

    const transactionsList = transactionsStringHandled.split("\r\n")

    console.log(transactionsList)
    // await createTransactionService(transactionsStringHandled, response)
    // const transactionsList = await readAllTransactionsService();

    // expect(transactionsList).toHaveProperty("map")
  });
});
